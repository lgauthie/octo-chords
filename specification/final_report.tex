...
and \cite{Papadopoulos:18}. All of these models use chromagrams (Pitch Class Profiles)
at the feature extraction phase, which represent the pitch content for a song for each sampled
window.  \cite{McVicar:00},  \cite{Lee:15}, \cite{Papadopoulos:18}, and\cite{Zenz:20} have all found
that chroma extraction on beats improves results, as chords more frequently change on beats.
...
and percussive sounds using harmonic/percussive sound seperation (HPSS). 


...EDIT: Write more about previous papers that used HPSS.
