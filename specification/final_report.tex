...
fft2chromamx.m written by Dan Ellis (http://www.ee.columbia.edu/~dpwe/resources/matlab/chroma-ansyn/#1). 
...
the reults by 6\%. In \cite{McVicor:00}, it is mentioned that this method exploits the stability
...
-{Jiang:21} investigates different preprocessing steps. Discusses that
...
Hidden Markov Models have become a common method for for automatic chrod estimation. In this
model, the a given chord depends on the previous chord predicted while all the other chords are hidden.
For each song in our data set, the set of chorma vectors are the observations, while the chord labels
are our states. 
...
\bibitem{Jiang:21}
Nanzhu Jiang, Peter Grosche, Verena Konz, and Meinard Müller.
"Analyzing Chroma Feature Types for Automated Chord Recognition"
{\it AES 42nd International Conference}
July 2011. 
