...
Median filters operate by replacing a given sample in a signal by the median of the signal values in a window around the sample \cite{FitzGerald:11} First step is short time fourier transform then median filtering with kernel sizes 1 and 23 followed by the inverse short time fourier transform. Input to the hiss is an N 
...
%\begin{figure}[p]
%   \centering
%    \includegraphics[width=0.8\textwidth]{hpssspec.png}
