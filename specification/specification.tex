...
While working on harmonic percussive source separation, we've found that the constant-Q
...
before performing median filtering. Unfortunately the inverse transform does not allow for perfect
...
Our feature extraction model requires that our bins are time aligned to the chord changes.
The CQT implementation we have does not include an inverse transform, and the features
extracted by the supplied code are not time aligned. Our fix will be to do a simple FFT followed
by median filtering in Python. After pre-processing is complete, the tracks will be put through
our feature extration model.
