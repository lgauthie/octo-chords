...
\cite{McVicar:00}. We plan to use HMMs to build our models.
...
min, aug, dim, sus2, sus4, X, or N, where X and N imply no chord is present.
For the scope of this project we will not try to classify inverted chords and
7th chords, and instead will classify them as one of the previously mentioned
classes.
...
In a spectrogram the harmonic components of a musical signal usually has a
...
the billboard dataset.  We will use the spectra2chroma and chromaFilter
...
Robert working on the audio preprocessing (ie\. Transient removal).  Lee and
